...
The engine is chosen by the deployment platform (domestic deployment → amap; international → Mapbox v6), not by the `country` value. You don't pick the engine.
...
The map stores and renders **WGS84 only** — submitted coordinates must already be WGS84.

- **Known GCJ-02 sources**: geolocation fields of Chinese table apps (飞书多维表格/Feishu Bitable, 钉钉/DingTalk, 企业微信/WeCom, 腾讯文档/Tencent Docs) and any Amap/高德/Tencent-sourced coordinates. **Known BD-09 sources**: Baidu data. Submitted raw, these land 100–700 m off in China with **no server-side error** — the numbers look valid.
- **Convert locally** with the CLI's `convert-coords` (pure local math — no request, no token): `--from gcj02|bd09`; two number columns via `--lng-col`/`--lat-col`, or one `"lng,lat"` string column via `--coord-col` (string shape preserved); single point via `--lng`/`--lat`. Input accepts a flat records array, canonical rows (`[{"cells": ...}]`, e.g. `fmt-rows` output), or an object with a `rows` array (`get_layer_data` response); rows with missing/non-numeric coordinates are counted and left untouched; `--out` may equal `--input`. Output is rounded to 6 decimals (~0.1 m).
- **Unknown origin — never convert blindly.** Plain lng/lat number columns of unknown origin could be either system, and a wrong conversion offsets as much as no conversion: ask the user where the coordinates came from, or submit as WGS84 and state the assumption in the delivery.
- **Already WGS84 — never convert again:** geocode output, and Mapbox-engine search results. Amap-engine search results are GCJ-02 (the envelope's `coordinateSystem` says which engine's frame you got).
