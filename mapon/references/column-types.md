# Field types: declaring data columns for map layers
...
`format: "currency"` without `currency` defaults to the deployment's money — **CNY** domestic, **USD** international. Pass the code explicitly when the money is anything else.
